...
// ExportCPFs attempts to extract all CPFs matches from a string
func ExportCPFs(s string) (matches []string) {
	matches = cpfRegexp.FindAllString(s, -1)
	return matches
}

// ExportCNPJs attempts to extract all CNPJs matches from a string
func ExportCNPJs(s string) (matches []string) {
	matches = cnpjRegexp.FindAllString(s, -1)
	return matches
}
